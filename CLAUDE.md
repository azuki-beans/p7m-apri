...
- `converter/validation.py` — **optional** eIDAS legal validation, separate from extraction. Where OpenSSL only confirms integrity, pyHanko establishes *trust*: it validates the signer's certificate against the EU Trusted Lists (LOTL, includes Italian AgID-accredited TSPs). API used: `async_validate_detached_cms()` on the enveloping SignedData, with a `ValidationContext` whose trust comes from `TSPTrustManager(tsp_registry=...)`, built once per process via `lotl_to_registry()` (restricted to `TRUST_LIST_TERRITORIES`, a **set of uppercase ISO codes** — default `{"IT"}` — passed as `only_territories` (pyHanko wants a `Set[str]`, not a string), so the cold-cache download is fast and skips foreign lists that trip parsing) + a `FileSystemTLCache` at `TRUST_LIST_CACHE_DIR` (default `/data/trust-lists` in Docker). `validate_signature(cms_bytes, level=1)` is a sync wrapper (`asyncio.run`) that **never raises** and returns a **list** of `ValidationOutcome` — one per `SignerInfo` (pyHanko validates a single signer at a time, so with parallel signatures `_validate` builds a `signed_data.copy()` holding one `SignerInfo` each); on any problem (no network, pyHanko/aiohttp missing, malformed file) the list holds a single `ValidationOutcome(available=False, error=...)`. Each outcome carries `level`, set by the caller: the view calls it once per entry of `ExtractResult.cms_layers` and flattens everything into `validations` for the template. On success `ValidationOutcome` also carries detail fields (signer org/country/ID, cert validity dates, digest/signature algorithm, eIDAS `qualified` label from `qualification_result`, QTSP `service_name`, and the full `chain` via `validation_path.iter_certs`), rendered in an optional `<details>` panel in `result.html`. pyHanko imports are deferred inside functions so the app boots and extraction works even without the `pyhanko[etsi,async-http]` dependency installed. The first validation is slow (downloads + verifies the EUTL); keeping that cache warm is the job of the **planned Celery module** (not yet built). Revocation strictness is `SIGNATURE_REVOCATION_MODE` (default `soft-fail`). The `ValidationContext` also sets `time_tolerance=SIGNATURE_TIME_TOLERANCE` seconds (default 60, up from pyHanko's 1s) so a server clock running behind doesn't make a freshly-fetched OCSP response fail as "too recent" (`InsufficientRevinfoError` / `RevinfoUsabilityRating.TOO_NEW`) — the real cure is NTP on the host, this just absorbs small skew.
- `converter/extractor.py` — the only place that shells out to OpenSSL. `_unwrap()` runs `smime -verify` on one CMS layer (see Core operation), reports `verified` from the presence of `"Verification successful"` in stderr, and collects **all** signer CNs of that layer (parallel signatures = multiple `SignerInfo`s) via `-signer <pem>` → `openssl crl2pkcs7 | openssl pkcs7 -print_certs`. `extract()` loops `_unwrap()` to handle **nested** `.p7m` (a signed file signed again: `doc.pdf.p7m.p7m`), at most `MAX_DEPTH` (10) levels, continuing only while the extracted content starts with the DER byte `0x30`; it returns `ExtractResult` with `layers` (`SignatureLayer(level, signers, verified)`, level 1 = outermost), `cms_layers` (raw CMS bytes of each level, used for per-level eIDAS validation), `verified` = all layers intact, and `signer` = comma-joined unique CNs (truncated to 255 for the model field). File type/extension is sniffed from magic bytes in `_detect()` — it only recognizes PDF/ZIP/DOC/RTF/XML; anything else falls back to `application/octet-stream` with no extension, so `_output_name()` keeps whatever was inside the `.p7m` name (stripping **every** trailing `.p7m`). `extract()` returns `None` (not an exception) when the first layer fails — invalid file, non-zero exit, or empty output.
- Two-step HTMX flow: `POST /verify/` extracts, stores the result row, and returns the `result.html` partial with a download link. The `validate` checkbox (off by default) additionally runs `validate_signature()` on every CMS layer and passes the flat list `validations` to the template, which groups them by `level` when there is more than one; the `Conversion` row also stores `signatures` (JSON list of `{level, signers, verified}`) rendered as the per-level list in `result.html`; `GET /download/<uuid>/` streams the bytes back. Extracted files are kept as a `BinaryField` BLOB in SQLite (model `Conversion`), **never written to disk**, and purged after 1h (`RETENTION`) by `_cleanup()` on each upload. The view rejects anything not ending in `.p7m` before calling `extract()`. Uploads are held entirely in memory and capped at 50 MB (`DATA_UPLOAD_MAX_MEMORY_SIZE` / `FILE_UPLOAD_MAX_MEMORY_SIZE` in `settings.py`).
- Migrations in `converter/migrations/` are checked in (`0001_initial.py` hand-maintained, `0002_conversion_signatures.py` generated) — if you change `models.py`, add a new migration with `makemigrations` (don't rewrite applied ones: the `/data` volume of existing deployments has them applied) so the Docker `migrate` step stays in sync.
